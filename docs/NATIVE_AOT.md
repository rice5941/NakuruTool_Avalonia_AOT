...
**既存 `JsonSerializerContext` の再利用 (sidecar 出力例)**

`RateGenerationCollectionJsonWriter` は生成 `.osu` の MD5 / メタを集計して ImportExport 互換の sidecar JSON を出力するが、新規 `JsonSerializerContext` は追加せず、`new ImportExportJsonContext(options)` でオプション付きに再構築した同コンテキストを使う。新しい出力面で ImportExport モジュールの型（`CollectionExchangeData` / `CollectionExchangeBeatmap`）を使いまわす際は、存在する `JsonSerializerContext` の再利用を優先してよい（重複 partial 生成を避け、リフレクションパスも増えない）。
...
`.osu` などのファイルから MD5 を求める場合は、`System.Security.Cryptography.MD5.HashData(Stream)` を `FileStream` に直接適用する。`MD5.Create()` のインスタンス化や `TransformBlock` / `TransformFinalBlock` 経由のリフレクション解決は使わず、`Convert.ToHexStringLower(hash)` で小文字 16 進文字列に変換するだけで AOT 安全に完結する。
