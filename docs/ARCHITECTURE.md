...
| NAudio | 2.2.1 | オーディオ処理（レート変換・WAV出力） |
| NAudio.Vorbis | 1.5.0 | Ogg Vorbis読み込み |
...
| SoundTouch | C/C++ | オーディオテンポ変更（SoundTouch 2.4.1, P/Invoke経由, LGPL-2.1） |
| LAME | C | MP3エンコード（LAME 3.100, libmp3lame.dll, P/Invoke経由, LGPL-2.1） |
...
├── BeatmapGenerator/  # レート変更譜面生成（NAudio + SoundTouch + LAME連携、DT/NCモード、MP3/OGG/WAV出力）
...
SoundTouch/            # SoundTouch ライブラリ（LGPL-2.1）
├── SoundTouch.cs      # C# P/Invoke ラッパー
├── SoundTouchDLL_x64.dll
└── COPYING.TXT

Lame/                  # LAME MP3エンコーダライブラリ（LGPL-2.1）
├── LameMp3Encoder.cs  # C# P/Invoke ラッパー（LibraryImport）
├── libmp3lame.dll
└── COPYING.TXT
...
graph TD
    ...
    BeatmapGenerator --> SoundTouchLib["SoundTouch (C/C++)"]
    BeatmapGenerator --> LameLib["LAME (C, libmp3lame)"]
