...
`Lame/LameMp3Encoder.cs` は LAME 3.100（libmp3lame.dll）のP/Invokeラッパーで、SoundTouch.cs と同様のパターン（IDisposable + ファイナライザ + ネスト NativeMethods）を採用している。

**NativeAOTとの統合ポイント**:

- `LibraryImport` 属性（Source Generator）による静的なP/Invoke宣言はNativeAOTで完全サポートされる
- `UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])` を指定し、ランタイムのマーシャリングオーバーヘッドを最小化
- `DllImport`（SoundTouch）と `LibraryImport`（LAME）の2パターンが共存しており、いずれもNativeAOT互換

| 設定項目 | 値 |
|---------|-----|
| DLL名 | `libmp3lame.dll` |
| 配置場所 | `Lame/LameMp3Encoder.cs` |
| パターン | `LibraryImport` + Source Generator |
| 呼び出し規約 | `CallConvCdecl` |
| リソース管理 | `IDisposable` + ファイナライザ（`lame_init` / `lame_close`） |
