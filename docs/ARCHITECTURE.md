...
| Avalonia.LinuxFramebuffer | 11.3.10 | Linux DRM/KMS フレームバッファランタイム（Raspberry Pi） |
...
| View | ViewModel | 備考 |
|------|-----------|------|
| `MainWindowView` | `MainWindowViewModel` | Desktop ウィンドウ |
| `MainContentView` | `MainWindowViewModel` | UI 本体（Desktop/DRM 共通） |
...
- `MainContentView` — `.Root<MainContentView>("MainContent")` として登録（DRM モード用）
...
**DRM モード（Raspberry Pi）**
```
Program.Main() → --drm 検出 → StartLinuxDrm
  → AppBuilder.Configure<App>()
    → App.Initialize()
      → AvaloniaXamlLoader.Load() / SemiTheme ロケール設定
    → App.OnFrameworkInitializationCompleted()
      → new Composition()
      → composition.MainContent
        → MainContentView + 全依存 ViewModel/Service がコンストラクタ注入で解決
          → MainContentView.Loaded → StartLoadingAsync
```
