...
## 3.3 純関数ユニットテスト — FfmpegArgumentsBuilderTests

`FfmpegArgumentsBuilderTests`（全 23 ケース）は `BeatmapGenerator` モジュールの `FfmpegArgumentsBuilder`（internal static 純関数クラス）を対象とした純粋なユニットテストである。スクリーンショット撮影やヘッドレス Avalonia ホストを伴わない通常の `[Fact]` / `[Theory]` テストで、組み立てた `string[]` の **順序と正確な要素** を `Assert.Equal` で検証する。

### 検証戦略

- **入出力契約の固定**: `FfmpegArgumentsBuilder` が公開する static メソッド（入力オーディオ変換用の `ffmpeg` 引数など）を、入力オプション→期待される `string[]` 引数列に 1:1 対応させてテスト
- **DT / NC モード両方をカバー**: DT モード（`atempo` フィルターチェーン、有効範囲 0.5–2.0 を超える倍率の多段分解）と NC モード（`asetrate` + `aresample`）をそれぞれ複数ケースで検証
- **出力コーデック分岐**: 入力拡張子（`.mp3` / `.ogg` / `.wav`）に応じた出力コーデック（`libmp3lame` / `libvorbis` / `pcm_s16le`）と VBR 品質（標準 `-q:a 4` / 高品質 `-q:a 0`）の切替を検証
- **チャンネル数による MP3→OGG フォールバック**: `AudioInputMetadataReader` で取得したチャンネル数が 3 以上の場合に出力形式が `.ogg` に切り替わる分岐を検証
- **ArgumentList 安全性**: 引数に空白や特殊文字を含むファイルパスを渡しても、シェルエスケープが不要な個別要素として組み立てられることを確認

純関数を対象とするため、subprocess の実行や `Process.Start` のモッキングは不要で、テストは高速・決定的に完了する。このレイヤで引数組立のリグレッションを抑え、実際の FFmpeg 実行を伴う結合テストの負担を下げる戦略を取っている。

---

## 3.4 レート生成 sidecar JSON 関連テスト

レート一括生成の ImportExport 互換 JSON 出力パスをカバーするため、以下の純関数・コンポーネントテストを追加している。いずれも Avalonia ヘッドレスホスト不要の通常 xUnit テスト。

| テストクラス | 対象 | 主な検証観点 |
|---|---|---|
| `OsuFileMetadataReaderTests` | `OsuFileMetadataReader.TryReadBeatmapSetId` / `TryReadBasicMetadata` | `[Metadata]` / `[Difficulty]` 以外のセクションでの早期打ち切り、セクション順に依存しないパース、不正値・キー未存在時の `false` 返却、I/O 例外のサイレント処理 |
| `BeatmapRateGeneratorJsonItemTests` | `BeatmapRateGenerator` の JSON アイテム構築 / `IncludedInOsz` 判定 | tempDir に生成された `.osu` の MD5 計算と `RateGenerationJsonItem` の保持、既存 `.osz` と `ZipArchiveMode.Update` でマージした際の同名 entry 衝突ケースで `IncludedInOsz == false` になることの確認 |
| `RateGenerationCollectionJsonWriterTests` | `RateGenerationCollectionJsonWriter.WriteBatchAsync` | `Success` / `JsonItem` / `IncludedInOsz` の組み合わせに応じた Skipped / Collision 集計、出力ファイル名規則（コレクション名 + rate label + タイムスタンプ）、出力 JSON が `ImportExportJsonContext.Default.CollectionExchangeData` でそのまま読み戻せる互換性 |
