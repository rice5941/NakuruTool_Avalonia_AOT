...
/// <summary>
/// ImportExportPageView 縺ｮ ViewModel
/// </summary>
public partial class ImportExportPageViewModel : ViewModelBase, IDisposable
{
        ...
        IImportExportService importExportService)
    {
        ...
        // 騾ｲ謐礼屮隕・        _importExportService.ProgressObservable
            .Subscribe(progress =>
            {
    ...
    {
        // 繧ｨ繧ｯ繧ｹ繝昴・繝医Μ繧ｹ繝・ DB 縺ｮ繧ｳ繝ｬ繧ｯ繧ｷ繝ｧ繝ｳ荳隕ｧ繧貞渚譏
        ...
        // 繧､繝ｳ繝昴・繝医Μ繧ｹ繝・ imports/ 繝輔か繝ｫ繝縺ｮ繝輔ぃ繧､繝ｫ荳隕ｧ繧貞渚譏
        ImportFiles.Clear();
    ...
    private async Task ImportAsync()
    {
        ...
        try
        {
            ...
            if (success)
            {
                // 繧､繝ｳ繝昴・繝亥ｾ後↓繧ｨ繧ｯ繧ｹ繝昴・繝医Μ繧ｹ繝医ｂ譖ｴ譁ｰ
                Initialize();
            }
        }
        catch (Exception ex)
        {
            await UpdateStatusAsync($"繧､繝ｳ繝昴・繝医お繝ｩ繝ｼ: {ex.Message}");
        }
        finally
        {
            await UpdateIsProcessingAsync(false);
