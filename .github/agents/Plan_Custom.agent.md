...
#tool:agent/runSubagent を使用して、コンテキストの収集と潜在的な障害の特定を行います。

1. **並列調査**: モジュールごとにサブエージェントを並列で起動し、タスクとの関連性を調査させます。
* このときに1つのモジュールに対してOpusエージェントとCodexエージェントの両方を起動し、同じプロンプトで異なる視点から調査させます。
2. **web検索の活用**: webから関連情報を収集する必要があると感じた場合は #tool:web/fetch を積極的に行ってください。
3. **変更箇所の特定**: どのモジュールに変更を入れるべきかの報告を受けます。
4. **ドキュメント精査**: サブエージェントに関連する docs ファイルを特定させ、その内容を読み込みます。

- agentName: _sub_Architect_opus
- agentName: _sub_Architect_codex

### Step 2: 計画 (Design & Refinement)
