...
| nakuru_stretch | Rust | オーディオタイムストレッチ（SignalsmithStretch, P/Invoke経由） |
...
├── BeatmapGenerator/  # レート変更譜面生成（NAudio + Rust FFI連携、DT/NCモード）
...
│   └── src/lib.rs
└── nakuru_stretch/    # Rust タイムストレッチライブラリ（SignalsmithStretch）
    └── src/lib.rs
...
graph TD
    ...
    BeatmapGenerator --> NativeStretch["nakuru_stretch (Rust)"]
