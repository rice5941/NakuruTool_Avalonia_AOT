...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| Avalonia | 11.3.12 | UIフレームワーク |
| Avalonia.Desktop | 11.3.12 | デスクトップランタイム |
| Avalonia.Fonts.Inter | 11.3.12 | Inter フォント |
| Avalonia.Diagnostics | 11.3.12 | デバッグ用DevTools（Debug構成のみ） |
| CommunityToolkit.Mvvm | 8.4.0 | MVVM Source Generator（`[ObservableProperty]`、`[RelayCommand]`） |
| HotAvalonia | 3.1.0 | XAML ホットリロード |
| Material.Icons.Avalonia | 2.4.1 | Material Design アイコン |
| Pure.DI | 2.3.3 | コンパイル時DI（Source Generator） |
| R3 | 1.3.0 | リアクティブプログラミング |
| R3Extensions.Avalonia | 1.3.0 | R3 の Avalonia バインディング |
| Semi.Avalonia | 11.3.7.3 | Semi Design テーマ |
| Semi.Avalonia.DataGrid | 11.3.7.3 | DataGrid テーマ |
| ZLinq | 1.5.5 | ゼロアロケーション LINQ |
...
| nakuru_rate_audio | Rust | オーディオレート変換（Bungee/Symphonia、P/Invoke経由） |
...
├── BeatmapGenerator/  # レート変更譜面生成（nakuru_rate_audio Rust FFI連携、DT/NCモード）
...
│   └── src/lib.rs
└── nakuru_rate_audio/  # Rust オーディオレート変換ライブラリ（Bungee/Symphonia）
    ├── src/lib.rs
    └── vendor/bungee/  # git submodule

docs/                  # ドキュメント群
imports/               # インポート用コレクションJSON
presets/               # 配布プリセット
settings/              # ローカル設定保存先
```
...
`SingleBeatmapGenerationViewModel` は独立ウィンドウではなく、`MapListPageView` 上の単体生成オーバーレイから利用される。
...
**Service / Feature Service（全て Singleton）**
...
- `IBeatmapDownloadService` → `BeatmapDownloadService`
...
graph TD
    ...
    BeatmapGenerator --> NativeRateAudio["nakuru_rate_audio (Rust/Bungee)"]
